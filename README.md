...
Narration is generated by Aivis Cloud API with the **阿井田 茂 / Calm** voice: a mature baritone chosen for the passage's restrained supernatural atmosphere. Generated MP3 files are stored under `public/assets/audio/{book}/{page}/layer-NN/`. The audio manifest hashes the text, model, style, and pacing settings so unchanged segments are not synthesized again.

To enable automatic narration:

1. Create an Aivis Cloud API key in the Aivis Cloud dashboard.
2. In this repository, open **Settings → Secrets and variables → Actions**.
3. Create a repository secret named `AIVIS_API_KEY`.
4. Run the Pages workflow or push a content change.

The deployment workflow generates changed audio before building the site, deploys that audio in the same run, and commits refreshed files back to the branch. If the secret is absent, the site still builds and the audio generator reports that narration is awaiting generation.

For a local generation run, expose `AIVIS_API_KEY` in the environment and run:

```bash
npm run generate:audio
```

The model, speaker, style, endpoint, and request interval can be overridden with `AIVIS_MODEL_UUID`, `AIVIS_SPEAKER_UUID`, `AIVIS_STYLE_ID`, `AIVIS_API_ENDPOINT`, and `AIVIS_REQUEST_INTERVAL_MS`.
